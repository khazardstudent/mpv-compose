...
cleanup_application_mk=""
if [ ! -f "$application_mk" ]; then
        application_mk="$builddir/Application.mk"
        cat >"$application_mk" <<'APP_MK'
APP_PLATFORM := android-24
APP_STL := c++_shared
APP_SUPPORT_FLEXIBLE_PAGE_SIZES := true
APP_MK
        cleanup_application_mk="$application_mk"
fi
...
if [ -z "$(pkg-config --cflags shaderc_combined)" ]; then
        ...
        [ -n "$cleanup_application_mk" ] && rm -f "$cleanup_application_mk"
        exit 1
fi
