...
static int common_set_property(JNIEnv *env, jstring jproperty, mpv_format format, void *value)
{
    ...
    if (result < 0)
        ...
        ALOGV("mpv_set_property(%s, %p) format %d", prop, value, format);
    env->ReleaseStringUTFChars(jproperty, prop);
...
jni_func(void, setPropertyInt, jstring jproperty, jint value) {
    ...
    common_set_property(env, jproperty, MPV_FORMAT_INT64, &int_value);
}

jni_func(void, setPropertyDouble, jstring jproperty, jdouble value) {
    double double_value = (double)value;
    common_set_property(env, jproperty, MPV_FORMAT_DOUBLE, &double_value);
}

jni_func(void, setPropertyBoolean, jstring jproperty, jboolean value) {
    int flag_value = value ? 1 : 0;
    common_set_property(env, jproperty, MPV_FORMAT_FLAG, &flag_value);
}
